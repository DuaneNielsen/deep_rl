...
def train(buffer, policy_net, optim, device='cpu', dtype=torch.float):
    ...
    dl = DataLoader(buffer, batch_size=10000, num_workers=0)

    for transitions in dl:
        state, action, G = algos.utils.to(transitions.s, transitions.a, transitions.i['g'], device=device, dtype=dtype)
        optim.zero_grad()
