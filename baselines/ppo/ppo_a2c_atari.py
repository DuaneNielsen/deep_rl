...
if __name__ == '__main__':
    ...
    if not config.silent:
        train_env = Plot(train_env, episodes_per_point=20, title=f'Train ppo-a2c-{config.env_name}',
                         history_length=200, refresh_cooldown=5.0)

    """ test env """
    test_env = make_env()
    if not config.silent:
        test_env = Plot(test_env, episodes_per_point=config.test_episodes, title=f'Test ppo-a2c-{config.env_name}',
                        history_length=200, refresh_cooldown=5.0)
    ...
    class A2CNet(nn.Module):
        ...
        def forward(self, state):
            ...
            output = self.mlp(l6.flatten(start_dim=1))

            value = output[..., 0:1]
            actions = softmax(output[..., 1:self.actions+1], dim=1)
            #actions = torch.cat([actions, 1 - actions], dim=1)
            a_dist = Categorical(actions)
            return value, a_dist
