...
if __name__ == '__main__':
    ...
    wandb.init(project=f"ppo-{config.env_name}", config=config)
    ...
    if not config.silent:
        train_env = Plot(train_env, episodes_per_point=config.episodes_per_batch, title=f'Train ppo-{config.env_name}')

    test_env = make_env()
    if not config.silent:
        test_env = Plot(test_env, episodes_per_point=1, title=f'Test ppo-{config.env_name}')
    ...
    policy_net = ppo.PPOWrapModel(policy_net)
    ...
    while total_steps < config.max_steps:
        ...
        """ train """
        ppo.train(train_buffer, policy_net, optim)
