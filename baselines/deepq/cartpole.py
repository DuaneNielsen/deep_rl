...
if __name__ == '__main__':
    ...
    wandb.init(project='cartpole_dqn', config=config)
    config = wandb.config
    ...
    env = Plot(env, episodes_per_point=1)
    ...
    for step_n, _ in enumerate(driver.step_environment(env, policy, render=True)):
        if step_n < config.episodes_per_batch:
            continue
        if step_n > 30000:
            break
        """ sample a batch and update """
        dqn.train(replay_buffer, q_net, optim, config.episodes_per_batch, discount=config.discount)
